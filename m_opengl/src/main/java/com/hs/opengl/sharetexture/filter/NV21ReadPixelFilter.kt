 ...
 * @author shun.he
 */
class NV21ReadPixelFilter(var context: Context, private var eglContext: EGLContext) :
    AbsFboFilter(context, R.raw.base_vert, R.raw.base_frag) {
    ...
    private var mDataListener: NV21ReadPixelDataListener? = null
    private var mGLWidthId = 0
    private var mGLHeightId = 0
    private var mPixelData: ByteBuffer? = null

    init {
        mGLWidthId = GLES20.glGetUniformLocation(program, "uWidth")
        mGLHeightId = GLES20.glGetUniformLocation(program, "uHeight")
    }

    fun startReadPixels(readPixelDataListener: NV21ReadPixelDataListener) {
        mDataListener = readPixelDataListener
        mPixelData =
            ByteBuffer.allocateDirect(mWidth * mHeight * 3 / 2).order(ByteOrder.nativeOrder())
        isStart = true
        createEglEnv()
    }
...
//                    mDataListener?.onNv21Data(
//                        imageRgba8888ToNv21(it, it.width, it.height),
//                        it.width,
//                        it.height
//                    )
//                    it.close()
...
//        }
    ...
    override fun onDraw(texName: Int, timestamp: Long): Int {
        if (!isStart) return texName
        yuvUtil!!.drawToTexture(texName)
        yuvUtil!!.vertexMatrix = mMtx
        yuvUtil!!.textureMatrix = mMtx
        yuvUtil!!.getOutput(mPixelData!!.array(), 0, mWidth * mHeight * 3 / 2)
        mDataListener?.onNv21Data(mPixelData!!.array(), mWidth, mHeight)
        return texName
    }



    private var yuvUtil:YuvOutputFilter? = null
    override fun setSize(width: Int, height: Int) {
        super.setSize(width, height)
        Log.e("setSize: ", "$width x $height")
//        GLES20.glUniform1f(mGLWidthId, mWidth.toFloat())
//        GLES20.glUniform1f(mGLHeightId, mHeight.toFloat())
        yuvUtil = YuvOutputFilter(YuvOutputFilter.EXPORT_TYPE_NV21)
        yuvUtil!!.create()
        yuvUtil!!.sizeChanged(width, height)
        yuvUtil!!.setInputTextureSize(width, height)
    }
...
interface NV21ReadPixelDataListener {
